...
// you use that seed number.
  ...
  char *output_file = read_string(argc, argv, "-o", "result-serial.txt");
  ...
  if(!in) {
    cerr << "could not read data file " << input_file << "\n";
    exit(1);
  }
  ntowns=0;
  do {
  ...
  output << genome.score() << endl;
  // output << "this is the distance from the sequence\n";
  output << genome;
  // output << ga.statistics() << "\n";
